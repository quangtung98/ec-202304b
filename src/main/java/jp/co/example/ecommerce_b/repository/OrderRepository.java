...
public class OrderRepository {
	...
	/**
	 * ordersとorder_itemsとorder_toppingsを結合したテーブルから取得したデータをList<Order>型のオブジェクトにセットするための定数
	 * order_itemsはitemsと、order_toppingsはtoppingsと結合されている
	 */
	private static final ResultSetExtractor<List<Order>> ORDER_LIST_ITEM_TOPPING_RESULTSET = (rs) -> {
		List<Order> orderList = new ArrayList<>();
		List<OrderItem> orderItemList = null;
		List<OrderTopping> orderToppingList = null;
		int beforeItemIdNum = 0;
		int beforeIdNum = 0;
		while (rs.next()) {
			int nowIdNum = rs.getInt("id");
			if (beforeIdNum != nowIdNum) {
				beforeIdNum = nowIdNum;
				Order order = new Order();
			...
			int nowItemIdNum = rs.getInt("order_item_id");
			if (nowItemIdNum != beforeItemIdNum) {
				beforeItemIdNum = nowItemIdNum;
				OrderItem orderItem = new OrderItem();
	...
	/**
	 * ユーザーIDで絞り込んで注文のリストを取得する.
	 * 
	 * @param userId ユーザーID
	 * @return 注文のリスト
	 */
	public List<Order> findAllByUserId(int userId) {
		String sql = ORDER_SELECT_SQL + "WHERE a.user_id =:userId ORDER BY order_item_id;";
